<template>
  <div>
    <v-card class="pb-4">
      <div class="mt-5 mx-5">
        <template
          v-if="_.includes(this.$auth.user().permissions, 'add_dosage')"
        >
    ...
    <v-card class="pb-4">
      <v-card-title>
        <div class="title">
          {{ $vuetify.lang.t("$vuetify.dosage.filter") }}
        </div>
      </v-card-title>
      <v-card-text>
        <v-row wrap>
                  ...
                  <template>
                    <v-tooltip bottom>
                      <template
                        v-slot:activator="{ on }"
                        v-if="
                          _.includes($auth.user().permissions, 'change_dosage')
                        "
                      >
                        <v-btn
                          @click="editDosage(item)"
                          color="secondary"
                          icon
                          text
                          v-on="on"
                    ...
                    <v-tooltip bottom>
                      <template
                        v-slot:activator="{ on }"
                        v-if="
                          _.includes($auth.user().permissions, 'delete_dosage')
                        "
                      >
                        <v-btn
...
export default {
  ...
  methods: {
    ...
    editDosage(item) {
      this.dialogForm.show = true;
      this.messages = {
        title: "$vuetify.dosage.update.title",
        submit: "$vuetify.dosage.update.submit",
        success: "$vuetify.dosage.update.success",
      };
      this.$refs.dosageForm.setForm(item);
    },
    performFilter() {
      let self = this;
