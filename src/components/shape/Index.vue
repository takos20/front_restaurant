  ...
  <div>
    <v-card class="pb-4">
      <v-row wrap>
        <div class="mt-5 mx-5">
          <template
            v-if="_.includes(this.$auth.user().permissions, 'add_shape')"
          >
    ...
    <v-card class="pb-4">
      <v-card-title>
        <div class="title">
          {{ $vuetify.lang.t("$vuetify.dci.filter") }}
        </div>
      </v-card-title>
      <v-card-text>
        <v-row wrap>
          ...
          <v-hover>
            <tbody slot-scope="{ hover }">
              ...
              <tr :key="item.id" v-for="item in items">
                ...
                <!--small-->
                <!--&gt;-->
                <!--{{ $vuetify.lang.t("$vuetify.shape.cols.billable." + item.billable) }}-->
                <!--</v-chip>-->
                <!--</template>-->
                <!--<template v-else>-->
                <!--<v-chip-->
                  ...
                  <v-tooltip top>
                    ...
                    <template>
                      <span v-if="!item.is_active">
                        {{
                          $vuetify.lang.t("$vuetify.shape.cols.status.enabled")
                        }}
                      </span>
                      <span v-else>
                        {{
                          $vuetify.lang.t("$vuetify.shape.cols.status.desabled")
                        }}
                      </span>
                    </template>
                  </v-tooltip>
                  ...
                  <template>
                    <v-tooltip bottom>
                      <template
                        v-slot:activator="{ on }"
                        v-if="
                          _.includes($auth.user().permissions, 'change_shape')
                        "
                      >
                        <v-btn
                          @click="editShape(item)"
                          color="secondary"
                          icon
                          text
                          v-on="on"
                    ...
                    <v-tooltip bottom>
                      <template
                        v-slot:activator="{ on }"
                        v-if="
                          _.includes($auth.user().permissions, 'delete_shape')
                        "
                      >
                        <v-btn
...
<script>
import FormEnable from "./EnableShape";
...
export default {
  ...
  data: () => ({
    ...
    loadingImport: false,
    messagesDelete: {
      success: "$vuetify.shape.delete.success",
    },
  }),
  methods: {
    upload() {
    ...
    setHeaders() {
      this.headers = [
        {
          text: "$vuetify.shape.cols.code.title",
          value: "code",
          align: "start",
          sortable: true,
          width: "50",
        ...
        {
          text: "$vuetify.shape.cols.name.title",
          value: "name",
          align: "start",
          sortable: true,
          width: "15",
        ...
        {
          text: "$vuetify.shape.cols.status.title",
          value: "is_active",
          align: "start",
          sortable: true,
          width: "1",
    ...
    create() {
      ...
      this.messages = {
        title: "$vuetify.shape.new.title",
        submit: "$vuetify.shape.new.submit",
        success: "$vuetify.shape.new.success",
      };
      this.form = {
        id: null,
        code: null,
    ...
    editShape(item) {
      this.dialogForm.show = true;
      this.messages = {
        title: "$vuetify.shape.update.title",
        submit: "$vuetify.shape.update.submit",
        success: "$vuetify.shape.update.success",
      };
      this.$refs.shapeForm.setForm(item);
    },
    enabled(item) {
      this.$refs.enableShape.setForm(item);
      if (!item.is_active) {
        this.dialogForm.shows.showInfo = true;
        this.messages = {
          title: "$vuetify.shape.cols.status.enabled",
          submit: "$vuetify.shape.update.submit",
          success: "$vuetify.shape.update.success",
        };
      } else {
        this.dialogForm.shows.showInfo = true;
        this.messages = {
          title: "$vuetify.shape.cols.status.desabled",
          submit: "$vuetify.shape.update.submit",
          success: "$vuetify.shape.update.success",
        };
      }
