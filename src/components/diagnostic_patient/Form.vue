...
export default {
  ...
  data: () => ({
    ...
    urlItemsDetails: "details_diagnostics",
  }),
  watch: {},
  created() {},
  validations() {
  ...
  methods: {
      ...
      let data = {
        diagnostic:
          this.formDetails.id === undefined
            ? this.formDetails.diagnostic
            : this.formDetails.diagnostic["id"],
    ...
    editDetails(item) {
      ...
      this.formDetails.diagnostic = item.diagnostic;
    ...
    getItemsDetails() {
      ...
      return new Promise((resolve, reject) => {
        ...
        self.$store
          .dispatch("request", {
            url: self.urlItemsDetails + "?diagnostic_patient=" + this.form.id,
            params: params,
          })
          .then((response) => {
    ...
    send() {
      ...
      return new Promise((resolve) => {
        self.$store
          .dispatch("request", {
            url: self.urlItemsDetails + "?diagnostic_patient=reset",
          })
          .then((response) => {
            // console.log("meta", data);
            resolve(response);
    ...
    resetDataFormRadiography() {
      this.formDetails.diagnostic = null;
    ...
    setForm(item) {
      ...
      this.form.diagnostic = item.diagnostic["id"];
      this.itemsDiagnostic.push(item.diagnostic);
  ...
  computed: {
    resultsErrors() {
      ...
      !this.$v.form.results.required &&
        errors.push(
          this.$vuetify.lang.t("$vuetify.diagnostic.cols.results.required")
        );
      return errors;
    },
  },
