        ...
        <v-form>
          <v-card-text>
            <v-row wrap>
              ...
              >
                <v-autocomplete
                  v-model="form.compose_ingredient"
                  :items="itemsComposeIngredient"
                  :loading="isLoadingCompose"
                  :search-input.sync="searchCompose"
                  color="primary"
                  item-text="name"
                  item-value="id"
                  :label="$vuetify.lang.t('$vuetify.compose_ingredient.title')"
                  class="font-weight-bold"
                  outlined
                  dense
                  clearable
              ...
              >
                <v-text-field
                  ...
                  :label="
                    $vuetify.lang.t(
                      '$vuetify.ingredient.cols.stock_quantity.title'
                    )
                  "
                  class="font-weight-bold"
                  outlined
...
export default {
  ...
  data: () => ({
    ...
    urlIngredient: "ingredients/all",
    urlComposeIngredient: "compose_ingredients/all",
    itemsIngredient: [],
    itemsComposeIngredient: [],
    isLoadingCompose: false,
    isLoading: false,
    search: null,
    searchCompose: null,
    itemsDish: [],
    urlItems: "compose_preparations",
    itemPerPage: 30,
    //itemPerPages: 3,
    itemPerPageOptions: [30, 40, 50],
    urlDeleteItems: "",
    sortDesc: true,
    urlItemsDetails_bills: "details_compose_preparations",
  }),
  watch: {
    pagination: {
      handler() {
    ...
    searchCompose(value) {
      ...
      const params = { name_language: value };
      let self = this;
      this.isLoadingCompose = true;
      this.$store
        .dispatch("request", {
          url: self.urlComposeIngredient,
          params: params,
        })
        .then((response) => {
          self.itemsComposeIngredient = response.data.content;
          // this.formData.quantity_served = 1;
          // console.log(this.form.product);
        })
        .finally(() => (this.isLoadingCompose = false));
    },
  },
  created() {
    this.setHeaders();
  ...
  methods: {
    ...
    createEmptySupply() {
      ...
      return new Promise((resolve) => {
        self.$store
          ...
          .then((response) => {
            self.compose_preparations = response.data.id;
            resolve(response);
    ...
    deleteEmptySupply() {
      ...
      return new Promise((resolve) => {
        self.$store
          .dispatch("request", {
            url:
              self.urlItems + "/" + this.compose_preparations + "/delete-empty",
            method: "DELETE",
          })
          .then((response) => {
            resolve(response);
    ...
    getDataFormdetails() {
      let data = {
        compose_preparation: this.compose_preparations,
        compose_ingredient: this.form.compose_ingredient,
        quantity: this.formData.quantity,
        stock_quantity: this.form.stock_quantity,
        total_amount: parseFloat(this.get_bills_amount || 0).toFixed(3),
    ...
    setForm(item) {
      ...
      this.compose_preparations = item.id;

      this.form.stock_quantity = item.stock_quantity;
      this.form.compose_ingredient = item.compose_ingredient;
      this.itemsComposeIngredient.push(item.compose_ingredient);
    ...
    getDetailsRoute() {
      if (this.compose_preparations) {
        return (
          this.urlItemsDetails_bills +
          "?compose_preparation=" +
          this.compose_preparations
        );
      } else {
        return this.urlItemsDetails_bills;
      }
    ...
    getDataForm() {
      let data = {
        ...
        stock_quantity: this.form.stock_quantity,
        compose_ingredient: this.compose_ingredients,
        compose_preparation: this.compose_preparations,
        total_amount: parseFloat(this.get_bills_amount || 0).toFixed(3),
        validate: true,
      };
    ...
    resetDataForm() {
      this.items = [];
      this.form.price_per_unit = null;
      this.form.unit = null;
      this.name_language = [];
      this.formData.quantity = null;
      this.form.stock_quantity = null;
      ...
      return new Promise((resolve, reject) => {
        ...
        self.$store
          ...
          .then((response) => {
            ...
            this.compose_preparations =
              self.items?.[0]?.compose_preparation?.["id"];
    ...
    saveRowEdit() {
      ...
      if (this.editedRow.compose_ingredient) {
        this.editedRow.compose_ingredient =
          this.editedRow.compose_ingredient["id"];
      }
      this.$store.dispatch("showNotification", {
        statut: false,
      });
    ...
    savePreparation() {
      ...
      if (!self.$v.$invalid) {
        ...
        this.$store
          .dispatch("request", {
            url: self.urlItemsDetails_bills + "/create",
            method: self.getMethodDetails(),
            data: self.getDataFormPreparation(),
          })
          .then(() => {
            self.$store.dispatch("showNotification", {
              statut: true,
  ...
  computed: {
    get_bills_amount() {
      ...
      this.items.forEach((d) => {
        total += d.cost; // Sum up the amount_net for each item
      });
    ...
    stock_quantityErrors() {
      let errors = [];
      if (!this.$v.form.stock_quantity.$dirty) return errors;
      !this.$v.form.stock_quantity.required &&
        errors.push(
          this.$vuetify.lang.t(
            "$vuetify.ingredient.cols.stock_quantity.required"
          )
        );
      return errors;
    },
