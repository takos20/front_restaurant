    ...
    <v-card class="pb-4">
      <v-toolbar dark color="#f5f5f5" dense>
        <div class="title" style="color: black">
          {{ $vuetify.lang.t("$vuetify.bills.view") }}
        </div>
        <v-spacer></v-spacer>
        <v-spacer></v-spacer>
          ...
          <v-row wrap>
            ...
            <v-col cols="12" sm="4" class="mb-0 py-0">
              <v-text-field
                ...
                outlined
                dense
                v-model="this.form.patient_code"
                readonly
                filled
              ></v-text-field>
            </v-col>
      ...
      >
        ...
        <!--          &lt;!&ndash;            dark&ndash;&gt;-->
        <!--          &lt;!&ndash;            @click="print_bills()"&ndash;&gt;-->
        <!--          &lt;!&ndash;            :disabled="this.get_bills_amount === 0"&ndash;&gt;-->
        <!--          &lt;!&ndash;          >&ndash;&gt;-->
        <!--          &lt;!&ndash;            {{ $vuetify.lang.t("$vuetify.bills.new.print_bills") }}&ndash;&gt;-->
        <!--          &lt;!&ndash;          </v-btn>&ndash;&gt;-->
        <!--        </v-card-actions>-->
...
export default {
  ...
  watch: {
    ...
    search(value) {
      ...
      this.$store
        .dispatch("request", {
          url:
            this.$auth.user()?.storage_depots != null
              ? self.urlProduct +
                "?storage_depots=" +
                this.$auth.user()?.storage_depots.id
              : self.urlProduct + "?storage_depots=null",
          params: params,
        })
        .then((response) => {
  ...
  methods: {
    ...
    setForm(item) {
      ...
      this.form.doctor = item.doctor["name"];
      this.form.cashier = item.cash["user"]["username"];
      this.form.patient_name = item.patient["name"];
      this.form.patient_code = item.patient["code"];
      ...
      this.form.patient_balance = item.patient_balance;
      this.form.bills_date = item.createdAt + " " + item.timeAt;
