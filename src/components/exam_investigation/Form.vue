            ...
            <v-container>
              <v-row wrap>
                ...
                <v-col cols="12" sm="5" class="mb-0 py-0">
                  <v-autocomplete
                    dense
                    v-model="form.type_exam_invest"
                    :items="itemsType_exam_invest"
                    :loading="isLoadingType_exam_invest"
                    :search-input.sync="searchType_exam_invest"
                    @blur="$v.form.type_exam_invest.$touch()"
                    @input="$v.form.type_exam_invest.$touch()"
                    :error-messages="type_exam_investErrors"
                    :label="
...
import { existTitleExamInvest } from "../../helpers/Validator";
...
export default {
  ...
  data: () => ({
    ...
    urlType_exam_invest: "type_investigations",
    isLoadingType_exam_invest: false,
    searchType_exam_invest: null,
    itemsType_exam_invest: [],
    numberRule: (val) => {
  ...
  watch: {
    searchType_exam_invest(value) {
      if (value === null) return;
      const params = { title: value };
      let self = this;
      this.isLoadingType_exam_invest = true;
      this.$store
        .dispatch("request", {
          url: self.urlType_exam_invest,
          params: params,
        })
        .then((response) => {
          self.itemsType_exam_invest = response.data.content;
        })
        .finally(() => (this.isLoadingType_exam_invest = false));
    },
  },
  ...
  methods: {
    ...
    getDataForm() {
      let data = {
        ...
        type_exam_invest: this.form.type_exam_invest,
    ...
    setForm(item) {
      ...
      this.form.type_exam_invest = item.type_exam_invest["id"];
      this.itemsType_exam_invest.push(item.type_exam_invest);
  ...
  computed: {
    ...
    type_exam_investErrors() {
      let errors = [];
      if (!this.$v.form.type_exam_invest.$dirty) return errors;

      !this.$v.form.type_exam_invest.required &&
        errors.push(
          this.$vuetify.lang.t(
            "$vuetify.exam_investigation.cols.type_investigation.required"
          )
        );
