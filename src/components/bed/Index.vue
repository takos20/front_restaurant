<template>
  <div>
    <v-card class="pb-4">
      <div class="mt-5 mx-5">
        <template v-if="_.includes(this.$auth.user().permissions, 'add_bed')">
    ...
    <v-card class="pb-4">
      <v-card-title>
        <div class="title">
          {{ $vuetify.lang.t("$vuetify.bed.filter") }}
        </div>
      </v-card-title>
      <v-card-text>
        <v-row wrap>
                  ...
                  <template>
                    <v-tooltip bottom>
                      <template
                        v-slot:activator="{ on }"
                        v-if="
                          _.includes($auth.user().permissions, 'change_bed')
                        "
                      >
                        <v-btn
                          @click="editRoom(item)"
                    ...
                    <v-tooltip bottom>
                      <template
                        v-slot:activator="{ on }"
                        v-if="
                          _.includes($auth.user().permissions, 'delete_bed')
                        "
                      >
                        <v-btn
...
export default {
  ...
  data: () => ({
    ...
    messagesDelete: {
      success: "$vuetify.bed.delete.success",
    },
  }),
  methods: {
    setHeaders() {
    ...
    create() {
      ...
      this.messages = {
        title: "$vuetify.bed.new.title",
        submit: "$vuetify.bed.new.submit",
        success: "$vuetify.bed.new.success",
      };
      this.form = {
        id: null,
        title: null,
    ...
    editRoom(item) {
      this.dialogForm.show = true;
      this.messages = {
        title: "$vuetify.bed.update.title",
        submit: "$vuetify.bed.update.submit",
        success: "$vuetify.bed.update.success",
      };
