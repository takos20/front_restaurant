    ...
    <v-card>
      ...
      <v-form style="margin-top: 15px; background-color: #eceff1">
        <v-card-text>
          <v-row wrap>
            ...
            <v-col class="mb-0 py-0" cols="12" sm="4" style="margin-top: -15px">
              <v-autocomplete
                ...
                "
                prepend-inner-icon="mdi-archive mdi-dark"
                class="font-weight-bold"
                outlined
                dense
                clearable
      ...
      <!-- Products Section - Compact -->
        <v-card 
          ...
          outlined 
          class="mb-2"
          :disabled="(!form.patient || !form.bill_type || !form.district) && (!patient || !form.bill_type || !district)"
        >
            ...
            >
              ...
              <template v-slot:item.actions="{ item }">
                <v-btn
                  x-small
                  color="primary"
                  :disabled="(!form.patient || item.qte_served_fake === 0 || get_subtotal(item) === 0) && (!patient || item.qte_served_fake === 0 || get_subtotal(item) === 0)"
        ...
        <div class="text-center mt-2">
          ...
          <v-card-text class="py-2 px-3">
            <div class="d-flex align-center mb-2">
              ...
              <span class="text-body-2 font-weight-medium">Détails ({{ items.length }})</span>
            </div>
        <v-data-table
          :footer-props="{
          ...
          <template v-slot:body="{ items }">
            <tbody>
              <tr :key="item.id" v-for="item in items">
                <td class="text-body-2" style="max-width: 150px">
      ...
      <v-card-actions class="py-2 px-3">
        ...
        <v-btn
          color="primary"
          :loading="loadingBtn"
          :disabled="(!form.bill_type || !form.district || !form.patient || !canEnableButton || this.get_bills_amount_net === 0) && (!form.bill_type || !district || !patient || !canEnableButton || this.get_bills_amount_net === 0)"
          @click="save_print_bills"
        >
          <v-icon left small>{{ form.id ? 'mdi-printer' : 'mdi-content-save' }}</v-icon>
          {{ form.id ? 'Imprimer' : 'Enregistrer' }}
        </v-btn>
      </v-card-actions>
...
<!-- Compact Footer -->
    ...
    ></form-settlement>
    <form-validate
      :dialog="dialogFormValidate"
      ref="validateForm"
      :messages="messages"
      @getItems="getPermission"
    ></form-validate>
    ...
    ></form-refund>
    <form-reduction
      :dialog="dialogFormReduction"
      ref="reductionForm"
      @closed="onRefundClosed"
      @getItems="getItemsDetails"
    ></form-reduction>
    <form-reduct
      :dialog="dialogFormReductionOther"
      ref="reductionOtherForm"
      @getItems="getItemsDetails"
    ></form-reduct>
    <delete-item
          :dialog="dialogDelete"
...
import FormReduction from "./warningReduction";
import FormRefund from "./warningRefund";
import FormReduct from "./warningReductionOther";
...
export default {
  ...
  data: () => ({
    ...
    itemPerPage: 6,
    itemsPerPageProduct: 5,
    // pageCat: 1,
    tva_collected: null,
    sortByCat: "id",
    //itemPerPages: 3,
    itemPerPageOptions: [1000],
    itemPerPageOptionsProduct: [5, 10, 30],
    urlDeleteItems: "",
    sortDesc: true,
    had_paid: false,
    loading: {
  ...
  methods: {
    getItemsProduct() {
      ...
      return new Promise((resolve, reject) => {
        this.loadingDetailsProduct.list = true;
        self.$store
          .dispatch("request", {
          ...
          .then(() => {
            self.loadingDetailsProduct.list = false;
            resolve();
          });
      });
    },
    ...
    performFilter() {
      ...
      if (self.form.storage_depots !== null) {
        params["storage_depots"] = self.form.storage_depots;
      }
      self._search = params;
