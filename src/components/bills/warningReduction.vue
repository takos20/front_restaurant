    ...
    <v-card class="pa-3">
      <v-alert border="left" colored-border type="success" elevation="2">
        ...
        <div
          ...
          v-if="
            this.reduction.type === 'FREE' &&
            this.reduction.should_notify === true
          "
        >
          <!-- {{ $vuetify.lang.t('$vuetify.promo.message_notiy') }} -->
          {{ $vuetify.lang.t("$vuetify.promo.message_notify_100") }}
        </div>
        <div
          class="text-center"
          style="font-size: 15px; line-height: 1.6"
          ...
          "
        >
          <!-- {{ $vuetify.lang.t('$vuetify.promo.message_apply') }} -->
          {{ $vuetify.lang.t("$vuetify.promo.message_apply_100") }}
        </div>
      </v-alert>
      <v-autocomplete
        v-if="
        ...
        item-value="id"
        return-object
        :label="$vuetify.lang.t('$vuetify.promo.choose_free')"
    ...
    <v-list-item>
      <v-list-item-content>
        ...
        <v-list-item-subtitle class="grey--text">
          {{ data.item.category || 'Plat du jour' }}
...
export default {
  ...
  methods: {
    setForm(item) {
      ...
      this.form.storage_depots = item.storage_depots;
    ...
    getDataFormdetails() {
      let data = {
        ...
        amount_gross: this.form.dish["price"],
        amount_net: 0,
        tva: 0,
      };
