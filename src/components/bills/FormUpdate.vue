<template>
  ...
  >
    ...
    >
      ...
      <!--    </div>-->
      <v-card class="pb-1" style="margin-top: 5px">
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-spacer></v-spacer>
          <v-row wrap>
      ...
      <v-card style="margin-top: 8px">
        <v-form
          ...
          :disabled="form.patient === null || form.additional_info === null"
          v-if="this.form.bill_type === 'ordinance'"
        >
          <br />
          <v-card-text>
          ...
          <v-card-text>
            <v-row wrap>
              ...
              <v-col
                ...
                sm="4"
                class="mb-0 py-0"
                v-if="this.form.bill_type === 'exam_laboratory'"
              >
                <v-autocomplete
      ...
      <v-card
        style="background-color: #e9eef4; margin-top: 10px"
        color="#E9EEF4"
      >
        <v-form
          :disabled="
            ...
            <v-row wrap>
              ...
              >
                <v-text-field
                  ...
                  class="font-weight-bold"
                  outlined
                  v-model="form.insurance_patient"
                  readonly
                  filled
                  dense
                ></v-text-field>
...
export default {
  ...
  data: () => ({
    ...
    urlExam_investigation: "exam_investigations",
  ...
  methods: {
    ...
    getDataForm() {
      let data = {
        ...
        cash: this.form.cash["id"],
        cashier: this.form.cash["user"]["id"],
        ...
        payment_method:
          this.form.amount_received === 0 ? "NO_PAY" : this.form.payment_method,
        ...
        insurance_patient: this.get_insurance_patient,
        amount_received:
    ...
    getDataFormdetails() {
      ...
      let data = {
        ...
        type_consultation:
          ...
          this.formData.type_consultation === null
            ? ""
            : this.formData.type_consultation["id"],
        checkup:
    ...
    setForm(item) {
      ...
      this.form.bills_amount = item.bills_amount;
      this.form.insurance_patient = item.insurance_patient;
      this.getItemsDetails();
    ...
    getItemsDetails() {
      ...
      return new Promise((resolve, reject) => {
        ...
        self.$store
          .dispatch("request", {
            url: self.urlItemsDetails_bills + "?bills=" + this.form.id,
            params: params,
          })
          .then((response) => {
            let data = response.data;
    ...
    getPrice() {
      ...
      params["other_code"] = this.get_product_code;
      params["type_patient"] = this.get_patient_shape;

      if (!this._.isEmpty(this._search)) {
        this._search = this._.merge(params, this._search);
      }
      return new Promise((resolve, reject) => {
        this.loadingPrice.list = true;
        self.$store
          .dispatch("request", {
            url: "prices",
            method: "get",
          ...
          .then((response) => {
            let data = response.data;
            self.pricePatient = null;
            if (data.content.length > 0) {
              self.pricePatient = data.content[0]["price"];
            } else {
              self.pricePatient = 0;
            }
            // this.formData.laboratory = null;
  ...
  computed: {
    ...
    // get_price_patient() {
    //   if (this.formData.laboratory !== null || this.formData.details_stock !== null || this.formData.medical_act !== null ||this.formData.radiography !== null || this.formData.type_consultation !== null || this.formData.surgery !== null || this.formData.diagnostic !== null|| this.formData.exam_investigation !== null || this.formData.checkup !== null){
    //     this.getPrice();
    //
    //   }
    //   return this.pricePatient;
    // },
    get_product_code() {
      let code = "";
      ...
      } else if (this.form.bill_type === "exam_laboratory") {
        code =
          this.formData.laboratory === null
            ? ""
    ...
    get_patient_solde() {
      // let patient_code = "";
      if (this.form.patient == null) {
        return 0;
      } else {
        if (this.form.patient.solde == null) {
          return 0;
        } else {
          return this.form.patient.solde;
        }
      }
    },
    ...
    get_refund() {
      ...
      if (this.form.advances === undefined) {
        ...
        let refund = "";
        refund = this.form.advances - this.get_bills_amount;
        if (refund < 0) {
          refund = 0;
        }
        return refund;
