<template>
  <v-dialog v-model="dialog.show" persistent max-width="1250px">
    <v-card>
...
export default {
  ...
  data: () => ({
    ...
    itemsRadiography: [],
    urlRadiography: "meetings",
    urlItemsDetails: "reports",
  }),
  watch: {},
  mounted() {
  ...
  methods: {
    ...
    downloadWithAxios(item) {
      let url =
        "http://" +
        this.$auth.user().server_ip +
        ":" +
        this.$auth.user().server_port +
        "/" +
        item.file;
    ...
    getUploadAudio() {
      return this.urlItemsDetails + "?meeting=" + this.form.id;
    },
    getRouteDetails() {
      if (this.form.id) {
    ...
    getItemsDetails() {
      ...
      return new Promise((resolve, reject) => {
        ...
        self.$store
          .dispatch("request", {
            url: self.urlItemsDetails + "?meeting=" + this.form.id,
            params: params,
          })
          .then((response) => {
    ...
    save() {
      ...
      if (!self.$v.$invalid) {
        ...
        formData.append("meeting", this.form.id);
        ...
        axios
          .post(this.urlItemsDetails + "?meeting=" + this.form.id, formData, {
            headers: {
    ...
    setForm(item) {
      console.log("item", this.itemsUpdate);
      // Données envoyées lors de la modification
      this.form.id = item.id;
      this.getItemsDetails();
      this.audio = null;
      this.audio =
        this.itemsUpdate[0].attachmentsAudio.length == 0
          ? null
          : this.itemsUpdate[0].attachmentsAudio[0].file
              .split("/")
              .slice(-1)[0];
      this.form.results = this.itemsUpdate[0].results;
      this.setItemsFile(this.itemsUpdate[0].attachments);
      this.form.date = item.createdAt + " " + item.timeAt;
      this.attachments = [];
      this.attachments = this.itemsUpdate[0].attachments;
