<template>
  <div style="margin-top: 25px">
    ...
    <v-card v-if="this.isInventory === false">
      <v-form>
        <v-card-text>
          <v-row wrap>
            ...
            <v-hover>
              <tbody slot-scope="{ hover }">
                <tr :key="item.id" v-for="item in items">
                  ...
                  <td class="subtitle-2">
                    {{ parseFloat(item.quantity2.toFixed(3)) }}
                  </td>
...
export default {
  ...
  watch: {
    ...
    searchDepot(value) {
      if (value === null) return;
      const params = { name_language: value, is_default: true };
      let self = this;
  ...
  methods: {
    loadWarehouse(value) {
      ...
      this.$store
        ...
        .then((response) => {
          this.form.warehouse_default = response.data.content[0];
          this.itemsSelectingWarehouse.push(response.data.content[0]);
          this.warehouse = response.data.content[0].name;
        })
        .finally(() => {});
    },
    getOptionsType() {
