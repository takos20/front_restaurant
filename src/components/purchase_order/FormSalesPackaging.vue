<template>
  <div style="margin-top: 25px">
      ...
      <v-form @submit.prevent="saveDetails" style="margin-top: 10px">
        <v-card-text>
          <v-row wrap>
            ...
            <v-col cols="12" sm="2" class="mb-0 py-0">
              <v-autocomplete
                v-model="formData.sales_packaging"
                :items="itemsSales_packaging"
                :loading="isLoadingSales_packaging"
                :search-input.sync="searchSales_packaging"
                color="primary"
                item-text="name"
                item-value="id"
                return-object
                :label="$vuetify.lang.t('$vuetify.price_level.title')"
                class="font-weight-bold"
                outlined
                dense
                clearable
    ...
    <form-suppliers :dialog="dialogFormSupplies"></form-suppliers>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    searchSales_packaging(value) {
      if (value === null) return;
      const params = { name: value, product_id: this.formData.product.id };
      let self = this;
      this.isLoadingSales_packaging = true;
      this.$store
        .dispatch("request", {
          url: self.urlSales_packaging,
          params: params,
        })
        .then((response) => {
          // console.log("respp", response.data.content);
          self.itemsSales_packaging = response.data.content;
        })
        .finally(() => (this.isLoadingSales_packaging = false));
    },
    search(value) {
      if (value === null) return;
  ...
  methods: {
    ...
    getDataFormdetails() {
      let data = {
        ...
        supply_amount: this.get_supply_amount,
        business_unit: this.formData.sales_packaging["id"],
