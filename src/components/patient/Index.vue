<template>
  <div>
    <v-card class="pb-4">
      <v-row wrap>
        <div class="mt-2 mx-5">
          <template v-if="_.includes($auth.user().permissions, 'add_patient')">
    ...
    <v-card class="pb-4">
      ...
      <v-card-text>
        <v-row wrap>
          ...
          <v-col class="mb-0 py-0" cols="12" sm="3">
            <v-text-field
              :label="$vuetify.lang.t('$vuetify.patient.cols.first_name.title')"
              v-model="filter.name"
          ...
          <v-hover>
            <tbody slot-scope="{ hover }">
              <tr :key="item.id" v-for="item in items">
                ...
                <td class="subtitle-2" v-else-if="item.gender == 'WOMAN'">
                  <v-btn
                    class="mr-2 my-2 white--text"
                    color="pink darken-5"
                    small
                  ...
                  <template>
                    <v-tooltip bottom>
                      <template
                        v-slot:activator="{ on }"
                        v-if="
                          _.includes($auth.user().permissions, 'change_patient')
                        "
                      >
                        <v-btn
                    ...
                    <v-tooltip bottom>
                      <template
                        v-slot:activator="{ on }"
                        v-if="
                          _.includes($auth.user().permissions, 'delete_patient')
                        "
                      >
                        <v-btn
...
export default {
  ...
  data: () => ({
    ...
    loadingImport: false,
    messagesDelete: {
      success: "$vuetify.patient.delete.success",
    },
  }),
  watch: {
    searchCity(value) {
  ...
  methods: {
    ...
    setHeaders() {
      this.headers = [
        ...
        // {
        //   text: "$vuetify.patient.cols.type_patient.title",
        //   value: "type_patient",
        //   align: "start",
        //   sortable: true,
        //   width: "30",
    ...
    editPatient(item) {
      this.dialogForm.show = true;
      this.messages = {
        title: "$vuetify.patient.update.title",
        submit: "$vuetify.patient.update.submit",
        success: "$vuetify.patient.update.success",
      };
