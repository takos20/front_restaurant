          ...
          <v-hover>
            <tbody slot-scope="{ hover }">
              <tr :key="item.id" v-for="item in items">
                ...
                <td class="subtitle-2" v-if="item.dci1 === null"></td>
                <td class="subtitle-2" v-else>{{ item.dci1.name }}</td>
                <!--                <td class="subtitle-2">{{ item.dci2.name }}</td>-->
                <td class="subtitle-2" v-if="item.shape === null"></td>
                  ...
                  <v-tooltip top>
                    ...
                    <template>
                      <span v-if="!item.is_active">
                        {{
                          $vuetify.lang.t(
                            "$vuetify.product.cols.status.enabled"
                          )
                        }}
                      </span>
                      <span v-else>
                        {{
                          $vuetify.lang.t(
                            "$vuetify.product.cols.status.desabled"
                          )
                        }}
                      </span>
                    </template>
                  ...
                  <template>
                    <v-tooltip bottom>
                      <template
                        v-slot:activator="{ on }"
                        v-if="
                          _.includes($auth.user().permissions, 'change_product')
                        "
                      >
                        <v-btn
                    ...
                    <v-tooltip bottom>
                      <template
                        v-slot:activator="{ on }"
                        v-if="
                          _.includes($auth.user().permissions, 'delete_product')
                        "
                      >
                        <v-btn
...
<script>
import FormEnable from "./EnableProduct";
...
export default {
  ...
  data: () => ({
    ...
    messagesDelete: {
      success: "$vuetify.product.delete.success",
    },
  }),
  watch: {
    pagination: {
  ...
  methods: {
    ...
    setHeaders() {
      this.headers = [
        ...
        // },
        {
          text: "$vuetify.product.cols.status.title",
          value: "is_active",
          align: "start",
          sortable: true,
          width: "1",
