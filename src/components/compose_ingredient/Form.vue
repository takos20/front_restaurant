            ...
            <v-icon :color="rule.saved ? 'green' : ''">
              ...
              <v-col cols="12" sm="5" class="mb-0 py-0">
                <v-text-field
                  ...
                  @blur="$v.form.stock_quantity.$touch()"
                  @input="$v.form.stock_quantity.$touch()"
                  :error-messages="stock_quantityErrors"
                  dense
                  type="number"
...
export default {
  ...
  data: () => ({
    ...
    urlDeleteItems: "",
    sortDesc: true,
    urlItemsDetails_bills: "details_compose_ingredients",
  }),
  watch: {
    pagination: {
      handler() {
  ...
  methods: {
    ...
    createEmptySupply() {
      ...
      return new Promise((resolve) => {
        self.$store
          ...
          .then((response) => {
            self.compose_ingredients = response.data.id;
            resolve(response);
    ...
    deleteEmptySupply() {
      ...
      return new Promise((resolve) => {
        self.$store
          .dispatch("request", {
            url:
              self.urlItems + "/" + this.compose_ingredients + "/delete-empty",
            method: "DELETE",
          })
          .then((response) => {
            resolve(response);
    ...
    getDataFormdetails() {
      let data = {
        ...
        total_amount: this.get_bills_amount,
    ...
    setForm(item) {
      ...
      this.compose_ingredients = item.id;
    ...
    getDetailsRoute() {
      if (this.compose_ingredients) {
        return (
          this.urlItemsDetails_bills +
          "?compose_ingredient=" +
          this.compose_ingredients
        );
      } else {
        return this.urlItemsDetails_bills;
      }
